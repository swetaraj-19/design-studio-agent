...
AGENT_WHL_FILE = "design_studio_agent-0.1.0-py3-none-any.whl"
FLAGS = flags.FLAGS
...
def create(env_vars: dict[str, str]) -> None:
    ...
    if not os.path.exists(AGENT_WHL_FILE):
        ...
        raise FileNotFoundError(f"Agent .whl file not found: {AGENT_WHL_FILE}")
    ...
    print(f"\n[✅ SUCCESS] Design Studio Agent deployed to: {remote_agent.resource_name}")
...
def main(argv: list[str]) -> None:
    ...
    logger.info(
        "Environment variables to be passed to agent: %s", 
        list(env_vars.keys())
    )
