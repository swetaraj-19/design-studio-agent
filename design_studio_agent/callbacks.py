...
async def before_root_agent_model_callback(
    ...
    try:
            ...
            for idx, part in enumerate(content.parts):
                if part.inline_data:
                    try:
                        ...
                        logger.error("Root agent callback error while processing inline data: %s", error, exc_info=True)
                        continue
                    ...
                    try:
                        ...
                        if part.function_response.name in [
                            ...
                            logger.error("Root agent callback error while processing func response: %s", error, exc_info=True)
                            continue
        ...
        logger.error("Error in before_root_agent_model_callback: %s", error, exc_info=True)
        
        return {
            "status": "error",
            "message": f"Error in before_root_agent_model_callback: {str(error)}"
        }
